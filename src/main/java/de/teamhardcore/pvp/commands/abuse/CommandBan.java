...
public class CommandBan implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        if (args.length < 2 || args.length > 3) {
            player.sendMessage(StringDefaults.PREFIX + "§cVerwendung§8: §7/ban <Spieler> <Grund>");
            return true;
        }
        ...
        if (args.length == 2) {
            if (args[1].equalsIgnoreCase("confirm")) {
                ...
                player.sendMessage(StringDefaults.PREFIX + "§eDu hast den Spieler erfolgreich vom Server gesperrt.");
            ...
            UUIDFetcher.getUUID(args[0], uuid -> {
                ...
                configuration.setType(AbuseType.BAN);
